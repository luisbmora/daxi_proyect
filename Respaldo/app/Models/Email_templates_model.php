...
class Email_templates_model extends Crud_model
{
    ...
    function get_templates($options = array())
    {
        ...
        if ($user_id) {
            $where .= " $mail_table.user_id='$user_id'";
        }

        $this->db->query('SET SQL_BIG_SELECTS=1');

        $sql = "select * FROM $mail_table where deleted=0 and " . $where;
        return $this->db->query($sql);
    }


    function get_emails($options = array())
    {
    ...
    function ci_save_new($template_name, $custom_message, $id)
    {
        $mail_table = $this->db->prefixTable('email_templates');
   ...
   function save_mail_send($options = array()){
    ...
    function save_data($id, $template_name, $email_subject, $custom_message)
    {
        $mail_table = $this->db->prefixTable('email_templates');
    ...
    function data_host_mail($options = array())
    {
