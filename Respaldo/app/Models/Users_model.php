...
class Users_model extends Crud_model {
    ...
    function update_assing($assing) {

        $delete="delete from _assing_status";
        $this->db->query($delete);

        for($i=0; $i<=sizeof($assing); $i++){
            $sql = "insert into _assing_status values(".$assing[$i].", 1, 0)";
            $this->db->query($sql);
        }
        return true;
    ...
    private function make_quick_filter_query($filter, $users_table) {
      ...
      function get_Users_Assing(){
         
        $sql = "SELECT
CONCAT( _users.first_name, ' ', _users.last_name ) AS fullName,
_users.job_title,
_users.id,
...
_users.`status` = 'active' and _users.deleted=0
GROUP BY
_users.id
";
