...
class Clients_model extends Crud_model {
    ...
    function get_leads($options=array()){
        ...
        LEFT JOIN _lead_source ON _clients.lead_source_id = _lead_source.id
        $join_custom_fieds               
    ...
    WHERE
        _clients.deleted =0". $where;
    
        return $this->db->query($sql);
    }

    function get_Clients_by_User($id){
    ...
    FROM
        ...
        INNER JOIN _users ON _clients.owner_id = _users.id 
    WHERE
        _clients.deleted =0 and _clients.owner_id=".$id;
        return $this->db->query($sql);
    }


    function get_detail_one($id){

        $this->db->query('SET SQL_BIG_SELECTS=1');     
        $sql = "SELECT  _clients.id,_clients.company_name,_clients.phone,_clients.owner_id,_clients.lead_source_id,_clients.last_lead_status,_users.email,_users.id as User_id
        FROM _clients  INNER JOIN _users ON _users.client_id = _clients.id WHERE  _clients.id =".$id." and  _clients.deleted =0";
        return $this->db->query($sql);
    }
