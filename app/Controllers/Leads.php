...
class Leads extends Security_Controller
{
    ...
    public function report_source()
    {
        ...
        $view_data['lead_statuses'] = $this->Lead_status_model->get_details()->getResult();
        $view_data['lead_sources'] = $this->_get_source_dropdown("filter");

        return $this->template->rander("leads/report_source", $view_data);
    }
    ...
    public function modal_form()
    {
        ...
        $view_data['user_info'] = $this->Users_model->get_details()->getResult();
    ...
    private function make_lead_modal_form_data($lead_id = 0) {
        ...
        $view_data['questions'] = $this->Lead_questions_model->get_details()->getResult();
    ...
    private function _get_source_dropdown($view_type = "")
    {
        $lead_combo = $this->Lead_source_model->get_all_where(array("deleted" => 0))->getResult();

        $lead_combo_dropdown = array();

        if ($view_type == "filter") {
            $lead_combo_dropdown = array(array("id" => "", "text" => "- " . app_lang("title") . " -"));
        }

        foreach ($lead_combo as $source) {
            $lead_combo_dropdown[] = array("id" => $source->id, "text" => $source->title);
        }

        return $lead_combo_dropdown;
    }
    ...
    function save_leads() {
       ...
       $data = array(
           ...
           "phone" => $phone, // $this->request->getPost('phone'),
           ...
           "question" => $this->request->getPost('lead_question_id')
    ...
    //     $ban=false;
       ...
       $verificar = mysqli_query($conexion, "SELECT * FROM _clients WHERE email = '".$email."' OR phone = '".$phone."'" ); 
        // return json_encode($verificar_email);
        if(mysqli_num_rows($verificar) > 0 ){
            $ban = true;
            echo json_encode(array("success" => false, 'message' => 'Email o Telefono registrado previamente'));
            return; 
        } 
       ...
       $old=$this->request->getPost('owner_old');
       ...
       $this->Users_model->ci_save($dataMail,$this->request->getPost('User_id'));
       ...
       if ($save_id) {
        ...
        //        if ($this->request->getPost('owner_old') != $this->request->getPost('owner_id')) {
        ...
        //            $ch = curl_init('https://grupodaxi.site/leads.php');               
        //            curl_setopt ($ch, CURLOPT_POST, 1);               
        //            curl_setopt ($ch, CURLOPT_POSTFIELDS, "Name=".$name."&Owner=".$owner_id."&Id=".$id);
    ...
    //     // $verificar_phone = mysqli_query($conexion, "SELECT * FROM _users WHERE phone = '".$phone."'");
        
    //     // if(mysqli_num_rows($verificar_phone) > 0 ){
    //         //     $ban = true;
    //         //     echo json_encode(array("success" => false, 'message' => 'Telefono registrado previamente'));
    //         //     // return 'Teléfono registrado previamente, intenta con otro diferente';
    //         //     return;
    //         // }     
    ...
    //     $save = $this->Users_model->ci_save($dataMail, $this->request->getPost('user_id'));
    ...
    //     //         echo json_encode(array("test" => $test, "data" => $data, "old" => $old));
    //     //     } else {
    //     //         echo json_encode(array("success" => false, 'message' => app_lang('error_occurred')));
    //     //     }
    //     // }
    ...
    public function list_data()
    {
        ...
        $list_data = $this->Clients_model->get_leads($options)->getResult();
    ...
    public function list_report_source()
    {
        ...
        $list_data = $this->Clients_model->get_client_source($options)->getResult();

        $result = array();
        foreach ($list_data as $data) {
            $result[] = $this->_make_row_source($data);
        }
    ...
    public function list_report()
    {
        ...
        $list_data = $this->Clients_model->get_detail_source($options)->getResult();

        $result = array();
        foreach ($list_data as $data) {
            $result[] = $this->_make_row_source_detail($data);
        }
    ...
    public function view_source($contact_id = 0, $tab = "")
    {
        $view_data['source_info'] = $this->Users_model->get_one($contact_id);
    ...
    private function _make_row_source($data)
    {

        return array(
            $data->id,
            $data->title,
            $row_data[] = modal_anchor(get_uri("leads/modal_form_source"), $data->count, array("class" => "edit", "title" => 'Conteo de leads', "data-post-id" => $data->id))

        );
    }

    private function _make_row_source_detail($data)
    {
    ...
    function modal_form_source(){

        $lead_id = $this->request->getPost('id');
        $view_data = $this->make_lead_modal_form_data($lead_id);

        return $this->template->view('leads/modal_form_source', $view_data);
    }
    ...
    public function import_leads_modal_form()
    {
        $this->access_only_allowed_members();
    ...
    public function save_lead_from_excel_file()
    {
        ...
        foreach ($excel_file as $key => $value) { //rows
            ...
            $dataNotes = array(
                ...
                "labels" => 2,
                "project_id" => 0,
                "client_id" => $lead_save_id,
                "user_id" => $this->login_user->id,
                "is_public" => 0,
                "id" => $lead_save_id,
            );

            $this->Notes_model->ci_save($dataNotes);
        }
    ...
    public function validate_import_leads_file()
    {
        $file_name = $this->request->getPost("file_name");
    ...
    public function validate_import_leads_file_data()
    {
        $table_data = "";
        $error_message = "";
        ...
        return json_encode(array("success" => true, 'table_data' => $table_data));
    }

    public function download_sample_excel_file()
    {
        $this->access_only_allowed_members();
        return $this->download_app_files(get_setting("system_file_path"), serialize(array(array("file_name" => "import-template-leads-prospects.xlsx"))));
    }
