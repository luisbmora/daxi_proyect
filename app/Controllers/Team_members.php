...
class Team_members extends Security_Controller {
    ...
    public function assign_list() {
        return $this->template->rander("team_members/index", 123);
    }
    ...
    private function _make_row($data, $custom_fields) {
        ...
        $assigned_link = "";
        if ($this->login_user->is_admin && $this->login_user->id != $data->id) {
            $assigned_link = js_anchor("<i data-feather='x' class='icon-16'></i>", array('title' => app_lang('job_assign'), "class" => "update", "data-id" => $data->id, "data-action-url" => get_uri("team_members/assign"), "data-action" => "assing-confirmation"));
        }

        $row_data[] = $assigned_link;
    ...
    function delete() {
        ...
        function assign() {
            ...
            $view_data['assing_list']=$this->Users_model->get_Users_Assing()->getResult();  
            return $this->template->view('team_members/list_assing', $view_data);
            ...
            //    echo "Error conectando a la base de datos.<br>"; 
             ...
             if (!mysqli_select_db($link, "grupodax_crm")) 
             { 
            ...
            concat( _users.first_name, ' ', _users.last_name ),
            _users.job_title,
            _users.id,
            ...
            FROM
            _users
            LEFT JOIN _assing_status ON _assing_status.id_user = _users.id
            WHERE
            _users.user_type='staff' and
            _users.deleted = 0 AND
            _users.first_name != ''
            ORDER BY
            _users.first_name ASC,
            _users.last_name ASC");
            
             echo  '<table class="table table-responsive table-bordered table-hover" style="width: 90%; color: #444;" align="center">';
             ?>
             <form name='frmAssing' method='post' action='prueba.html' id='frmAssing'>
             <?php
             while($row=mysqli_fetch_array($query)){
                ...
                echo "<tr>";
                    ...
                    ?>
                    <td><input type='checkbox' id='chkbxAssing' name='assing' <?=$checked;?> class='chkbxAssing' value="<?=$value;?>" oonclick="valuesAssing()"></td>
                    <?php
            ...
            <button type="submit" class="btn btn-primary" style="margin: 0px 0px 0px 83%;"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-check-circle icon-16"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg> Guardar</button></form>
...
*/
    ...
    function save_general_info($user_id) {
        ...
        //save general information of a team member
        function save_assing() {

            //for($i=0; $i <= count($this->request->getPost('assing')); $i++){      

                $assing= $this->request->getPost('assing');
                    $this->Users_model->update_assing($assing);
              //  $assing=0;
    ...
    /* lista de archivos, preparada para la tabla de datos */
