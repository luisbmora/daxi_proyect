...
$campany = htmlspecialchars($_POST["Campaña"]);
...
/*
$chanel=mysqli_query($conn, "select * from _lead_source where title like '%".$campany."%'");
$row_cnt = mysqli_num_rows($chanel);


if($row_cnt==0){
    mysqli_query($conn, "INSERT INTO `_lead_source` VALUES (null, '".$campany."', 0,0)");
    $CanalID = mysqli_insert_id($conn);
}else{
 while($rowCh=mysqli_fetch_array($chanel)){
    $CanalID =$rowCh[0];
 }
}*/
...
//validar numero de fin a l inicio y correo


mysqli_query($conn, "insert into _clients (company_name,address, city,created_date,website, phone,starred_by,group_ids,is_lead,lead_status_id,owner_id,created_by,lead_source_id,last_lead_status,client_migration_date) values ('".$company_name."', '".$address."', '".$city."' , NOW(), '".$email."', '".$phone."', '', '',1,1,".$id_User.",".$id_User.",".$CanalID.",1,NOW())");
...
if(htmlspecialchars($_POST["Financiamiento"]) != ''){
    mysqli_query($conn, "INSERT INTO `_notes` VALUES (null, 1, NOW(), '".$_POST['Financiamiento']."', '', 0, ".$ClientID.", ".$UserID.", '', 'a:0:{}', 0, 0);");
}
...
try {
    ...
    // Contenido
    $mail->isHTML(true);                          
    $mail->Subject = 'Leads de facebook GrupoDaxi';

    $mail->Body = '<div style="background-color: #eeeeef;padding: 50px 0; background-image: url(https://www.grupodaxi.site/files/system/system_file61f2d4eed78ff-MURO-ID---GLORIETA-CON-MUKTANO-ACT.jpg);">
    <div style="max-width:640px; margin:0 auto; ">
        ...
        <div style="padding: 20px; background-color: rgb(255, 255, 255);">
            ...
            <p style=""><span style="color: rgb(85, 85, 85); font-size: 14px; line-height: 20px;"><span
                        style="font-weight: bold;">Se ha puesto en contacto un lead nuevo mediante Facebook.com</span></span></p>
            <br>
            <p style=""><span style="color: rgb(85, 85, 85); font-size: 14px; line-height: 20px;">
                    Nombre: '.$company_name.'<br>
                    Télefono: '.$phone.'<br>
                    Email: '.$email.'<br>
                    Financiamiento: '.htmlspecialchars($_POST["Financiamiento"]).'<br>
                    Mensaje: '.htmlspecialchars($_POST["Mensaje"]).'<br>
                    Formulario: Facebook: '.$campany.'
                </span></p>
